...
def generate(library_root, output):
    ...
    def add(ref, lib, value, nets, schematic, pcb, footprint, angle=0, side="F", mpn="", dnp=False):
    ...
    add("J2", "Connector_Generic:Conn_01x09", "DISPLAY HARNESS / 3V3",
        {str(i+1): n for i, n in enumerate(j2)}, (330, 95), (68, 35), header.format(9))
    ...
    for ref, supply, value, sch, pos in [
        ("C1", "+3V3", "10u 10V X7R", (80, 200), (74, 30)),
        ("C2", "+3V3", "100n 16V X7R", (130, 200), (79, 30)),
    ]:
    ...
    for ref, signal, value, sch, pos, dnp in [
        ("R1", "GPIO13", "4.7k DNP", (220, 200), (74, 40), True),
        ("R2", "GPIO14", "4.7k DNP", (285, 200), (79, 40), True),
    ...
    for index in range(18):
        ...
        pcb_x, pcb_y = panel_x - 3.0, 113.0 - panel_y + rear_extension
        ...
        add(f"SW{index+1}", "Switch:SW_Push", f"KEY_{index+1} MX HOTSWAP",
            ...
            "Workbench:keyswitch_cherrymx_hotswap_1u", side="B")
        ...
        add(f"D{index+1}", "Device:D", "1N4148W / K TO ROW",
            ...
            "Diode_SMD:D_SOD-123", side="B", mpn="1N4148W")
    ...
    schematic = node("kicad_sch", node("version", 20250114), node("generator", "eeschema"),
                     ...
                     node("title_block", node("title", "Kivo Workbench"),
                          node("rev", "S3 r01 DRAFT"), node("date", "2026-09-05")))
    ...
    for index, (text, position) in enumerate([
        ...
        ("YD ESP32-S3 SOCKETED CARRIER. Reuse native USB-C. Firmware display + diode-matrix rollover support pending.", (35, 394)),
    ]):
    ...
    (output / "placement.json").write_text(json.dumps(dict(
        name=NAME, sheet_uuid=root_id, width=126, height=105+rear_extension,
        rear_extension_mm=rear_extension, matrix_rows=rows, matrix_columns=columns,
