...
- **功能开关门控**：Hardware Profile 可把一个常开/常闭 GPIO 开关绑定到若干按钮；开关未处于启用状态时，这些按钮不会执行动作。
...
功能开关属于 Hardware Profile 的输入源，不会出现在按键布局中。配置时选择开关 GPIO、常态（断开或闭合）、按钮启用状态和受影响的按钮。运行时 Kivo 以开关的实时状态门控新动作；开关关闭时不会计入按键动作统计，正在执行的动作会完整结束。
