...
__xdata uint8_t keyboardIdle = 0;
  ...
  if (len == (sizeof(USB_SETUP_REQ))) {
        ...
        USB_REQ_TYP_STANDARD) // Not standard request
    {
      ...
      case USB_REQ_TYP_CLASS: {
        if (UsbSetupBuf->wIndexL == INTERFACE_ID_HID) {
          switch (SetupReq) {
          case HID_SET_IDLE:
            keyboardIdle = UsbSetupBuf->wValueH;
            break;
          case HID_GET_IDLE:
            Ep0Buffer[0] = keyboardIdle;
            len = SetupLen ? 1 : 0;
            break;
          ...
          case HID_GET_PROTOCOL:
            ...
            break;
          case HID_GET_REPORT:
            len = SetupLen < 8 ? SetupLen : 8;
            for (__data uint8_t i = 0; i < len; i++) Ep0Buffer[i] = Ep3Buffer[i];
            break;
          case HID_SET_REPORT:
            break;
          default:
    ...
    } else // Standard request
    {
      ...
      case USB_GET_DESCRIPTOR:
        ...
        case 3:
          ...
          break;
        case 0x21:
          pDescr = (__code uint8_t *)&ConfigurationDescriptor.HID_KeyboardHID;
