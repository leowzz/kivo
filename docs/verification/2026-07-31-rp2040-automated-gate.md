...
Final tested source commit: `e43bee7` (`fix: restore runtime protocol acceptance`). Evidence-only commits after this SHA do not change the tested runtime, firmware, or UI implementation.

| Check | Command | Result |
| --- | --- | --- |
| Complete non-destructive gate | `rtk test make test` | PASS: release/upload contracts, 30 native firmware cases, Rust unit and four-Device integration tests, Clippy `-D warnings`, 115 frontend tests, and production frontend build |
| ESP32-S3 acceptance build | `rtk proxy env KIVO_FIRMWARE_BUILD_ID=0.1.0+acceptance uv run pio run -e esp32s3` | PASS in 0.68 s; `.pio/build/esp32s3/firmware.bin` exists |
| RP2040 acceptance build | `rtk proxy env KIVO_FIRMWARE_BUILD_ID=0.1.0+acceptance uv run pio run -e rp2040` | PASS in 1.28 s; `.pio/build/rp2040/firmware.uf2` exists |
| macOS application bundle | `rtk npm run tauri build -- --bundles app` | PASS; produced `src-tauri/target/release/bundle/macos/Kivo.app` |
