...
- `kivo_integrated_workstation_sloped_panel.stl`: removable `132 x 118` key and
  display panel. Its underside is completely flat for support-free printing.
- `kivo_integrated_workstation_bottom_cover.stl`: removable bottom cover with
  the controller tray, cable-tie slots, and ventilation slots.
...
- 14 M3 heat-set brass inserts with the measured `3.9` body, `4.9` knurled
  ...
  4 for the bottom cover. Each receiving part has a `4.0 x 5.4` blind bore and
  shallow `5.1` lead-in.
...
- 2 small cable ties for the RP2040 or ESP32-S3 board.

1. Place the controller USB end toward the wide front connector opening and
   secure the board through the two cable-tie slots.
...
6. After testing the keypad, display, and handset switch, heat four inserts
   into the chassis bottom bosses and fasten the bottom cover from outside.
