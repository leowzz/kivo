...
export function AdvancedSettings(props: AdvancedSettingsProps) {
  ...
  return <div className="advanced-settings">
    ...
    {section === "profiles" && <ProfileManager language={props.language} profiles={props.profiles} editorProfileId={props.editorProfileId} devices={props.devices} onCreate={props.onCreate} onSelect={props.onSelectProfile} onImport={props.onImport} onExport={props.onExport} onDelete={props.onDelete} />}
    ...
    {section === "io" && target && <section className="advanced-editor">{offline && <p className="advanced-offline-note">{t(props.language, "advanced.offline")}</p>}<HardwareMapping language={props.language} layout={target.profile} hardwareProfiles={target.hardware_profiles} boardProfiles={props.boardProfiles} devices={offline ? [] : props.devices} learning={props.selectedDevice?.learning ?? null} initialHardwareProfileId={hardwareId} initialDeviceId={props.selectedDevice?.deviceId ?? null} selectedButtonId={selectedButtonId} onSelectButton={setSelectedButtonId} onChange={(nextHardwareProfiles) => props.onRequestProfileMutation((current) => ({ ...current, hardware_profiles: nextHardwareProfiles }))} onSelectionChange={() => undefined} onBeginLearning={props.onBeginLearning} onEndLearning={props.onEndLearning} /></section>}
    {section === "technical" && <section className="advanced-technical">{props.selectedDevice?.runtimeAssignment ? <dl><dt>{t(props.language, "devices.id")}</dt><dd>{props.selectedDevice.deviceId}</dd><dt>{t(props.language, "devices.serial")}</dt><dd>{props.selectedDevice.hardwareSerial}</dd><dt>{t(props.language, "devices.board")}</dt><dd>{props.boardProfiles.find((item) => item.id === props.selectedDevice?.boardProfileId)?.displayName ?? props.selectedDevice.boardProfileId}</dd><dt>{t(props.language, "devices.firmware")}</dt><dd>{props.selectedDevice.firmwareBuildId ?? "-"}</dd><dt>{t(props.language, "advanced.protocol")}</dt><dd>{props.selectedDevice.firmwareProtocol ?? "-"}</dd><dt>{t(props.language, "devices.port")}</dt><dd>{props.selectedDevice.port ?? "-"}</dd></dl> : <p className="advanced-empty">{t(props.language, "advanced.noDevice")}</p>}</section>}
    {target && <ConfigurationSettingsDialog open={settingsOpen} profile={target} sharedDeviceCount={sharedCount} language={props.language} onSave={(settings) => { props.onRequestProfileMutation((current) => ({ ...current, trigger_settings: settings })); setSettingsOpen(false); }} onDuplicate={(name) => props.onDuplicateForDevice(target, name)} onCancel={() => setSettingsOpen(false)} />}
  </div>;
}
