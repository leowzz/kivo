...
export function ConfigurationSettingsDialog({
  ...
  const [settings, setSettings] = useState<TriggerSettings>(
    profile.trigger_settings,
  );
  const [longPressSeconds, setLongPressSeconds] = useState(() =>
    formatSeconds(profile.trigger_settings.long_press_ms),
  );
  const [doublePressSeconds, setDoublePressSeconds] = useState(() =>
    formatSeconds(profile.trigger_settings.double_press_ms),
  );
  ...
  const validation = useMemo(() => {
    ...
    if (!Number.isInteger(long) || !Number.isInteger(double))
      return t(language, "settings.invalidSecondsError");
    if (long < 100 || long > 5000 || double < 100 || double > 1000)
      return t(language, "settings.secondsRangeError");
    return null;
  }, [language, settings]);
  ...
  return (
    <div className="modal-backdrop" role="presentation">
      <section
        className="device-setup-dialog configuration-settings-dialog"
        role="dialog"
        aria-modal="true"
        aria-labelledby="configuration-settings-title"
      >
        <header className="device-setup-header">
          <h2 id="configuration-settings-title">
        ...
        <div className="device-setup-body">
          <div className="settings-fields">
            <label>
              ...
              <input
                aria-label={t(language, "settings.longPressSeconds")}
                type="number"
                min={0.1}
                max={5}
                step={0.001}
                ...
                onChange={(event) => {
                  ...
                  const next = {
                    ...settings,
                    long_press_ms: parseSeconds(value),
                  };
                  setLongPressSeconds(value);
                  setSettings(next);
                  onDraftChange?.(next);
                }}
              />
            ...
            <label>
              ...
              <input
                aria-label={t(language, "settings.doublePressSeconds")}
                type="number"
                min={0.1}
                max={1}
                step={0.001}
                value={doublePressSeconds}
                onChange={(event) => {
                  const value = event.target.value;
                  const next = {
                    ...settings,
                    double_press_ms: parseSeconds(value),
                  };
                  setDoublePressSeconds(value);
                  setSettings(next);
                  onDraftChange?.(next);
                }}
              />
          ...
          {allowDuplicate && (
            <div className="settings-duplicate">
              <label>
                ...
                <input
                  aria-label={t(language, "settings.duplicateName")}
                  value={copyName}
                  onChange={(event) => setCopyName(event.target.value)}
                />
              </label>
              <p className="form-hint">
                {t(language, "settings.duplicateHint")}
              </p>
            </div>
          )}
        </div>
        <footer className="device-setup-footer">
          ...
          {allowDuplicate && (
            <button
              ...
              type="button"
              disabled={!copyName.trim() || Boolean(validation) || busy}
              onClick={async () => {
                setBusy(true);
                try {
                  await onDuplicate(copyName.trim());
                } finally {
                  setBusy(false);
                }
              }}
