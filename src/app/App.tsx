...
export default function App() {
  ...
  const productConfigHistory = useProductConfigHistory([], {
    maxSnapshots: 50,
  });
  ...
  const setupVerificationTimerRef = useRef<ReturnType<
    typeof setTimeout
  > | null>(null);
  ...
  const replaceRegistrySnapshot = useCallback(
    (snapshot: AppSnapshot, preserveProductDrafts = false) => {
      registryEpochRef.current += 1;
      const visibleProductConfigurations = preserveProductDrafts
        ? snapshot.productConfigurations.map(
            (config) => productConfigHistory.get(config.id) ?? config,
          )
      ...
      const visibleDevices = preserveProductDrafts
        ? snapshot.devices.map((device) => {
            const config = device.productConfigurationId
              ? visibleProductConfigurationsById.get(
                  device.productConfigurationId,
                )
              : undefined;
            return config ? { ...device, productConfig: config } : device;
          })
        : snapshot.devices;
      productConfigHistory.sync(
        productConfigHistoryEntries(visibleProductConfigurations),
      );
      setRegistry((current) => ({
        ...current,
  ...
  const applySnapshot = useCallback(
    (
      ...
      if (preserveHistory) {
        ...
        productConfigHistory.reset(
          productConfigHistoryEntries(snapshot.productConfigurations),
        );
      }
      setRegistry({
      ...
      const defaultDeviceId =
        visibleDevices.find((device) => device.connection === "online")
          ?.deviceId ??
    ...
    [
      productConfigHistory.get,
      productConfigHistory.reset,
      productConfigHistory.sync,
      profileHistory.reset,
      profileHistory.sync,
    ],
  );
  ...
  const requestForgetManagedDevice = useCallback(
    (deviceId: string) => {
  ...
  const selectManagedProductConfiguration = useCallback(
    async (deviceId: string, configurationId: string) => {
      try {
        const snapshot = await queue.enqueue(() =>
          invoke<AppSnapshot>("select_product_configuration", {
            deviceId,
            configurationId,
          }),
        );
  ...
  const createManagedProductConfiguration = useCallback(
    async (request: {
      deviceId: string;
      name: string;
      copyCurrent: boolean;
      ...
      try {
        const snapshot = await queue.enqueue(() =>
          invoke<AppSnapshot>("create_product_configuration", {
            request: {
  ...
  const autosaveProfiles = useMemo(
    () =>
      dirtyProfiles.filter((profile) => {
        ...
        const isCurrentlyShared =
          devices.filter(
            (device) =>
              device.runtimeAssignment?.device_profile_id === profileId,
          ).length > 1;
        return (
    ...
    [boardProfiles, devices, dirtyProfiles, pendingSharedDraftProfileIds],
  );
  ...
  const prepareSetupProfile = useCallback(
    async (
      deviceId: string,
      deviceName: string,
      ...
      const targetDevice = registry.devices.find(
      ...
      const compatibleSources = registry.deviceProfiles.filter((profile) =>
        profile.hardware_profiles.some(
          (hardware) =>
            hardware.board_profile_id === targetDevice.boardProfileId,
        ),
      );
      const requestedSource = sourceProfileId
        ? compatibleSources.find(
            (profile) => profile.profile.id === sourceProfileId,
          )
        : null;
      if (sourceProfileId && !requestedSource)
        throw new Error("hardware_resolution_required");
      ...
      const source =
        requestedSource ??
        compatibleSources.find((profile) =>
          profile.profile.groups.some((group) => group.buttons.length > 0),
        ) ??
        null;
      const createdSnapshot = await createDeviceProfile(
        source
          ? {
              kind: "clone",
              name: t(language, "setup.dedicatedProfileName", {
                name: deviceName,
              }),
      ...
      const firstButton = created.profile.groups.flatMap(
        (group) => group.buttons,
      )[0];
          ...
          : firstButton
            ? {
                [firstButton.id]: {
                  ...emptyTriggerActions(),
                  press: [
                    { type: "paste", text: t(language, "setup.sampleText") },
                  ],
                },
              }
            : {},
      ...
      if (JSON.stringify(prepared) !== JSON.stringify(created)) {
        if (PREVIEW_MODE) {
          ...
          const savedSnapshot = await invoke<AppSnapshot>(
            "save_device_profile",
            {
              profile: prepared,
            },
          ...
          ({ payload }) => {
            ...
            if (payload.input && payload.pressed !== null) {
              ...
              const eventIdentity = emittingDevice
                ...
                : null;
              const eventProfile =
                productTarget?.profile ??
                (eventIdentity
                  ? (profileDraftsRef.current.get(
                      eventIdentity.deviceProfileId,
                    ) ??
                    profilesRef.current.find(
                      (profile) =>
                        profile.profile.id === eventIdentity.deviceProfileId,
                    ))
                  : undefined);
              if (
                ...
                const eventHardware = eventProfile.hardware_profiles.find(
                ...
                if (buttonId) {
                  ...
                  if (
                    ...
                    const label =
                      eventProfile.profile.groups
                        .flatMap((group) => group.buttons)
                        .find((button) => button.id === buttonId)?.label ??
                      buttonId;
                    ...
                    const actionCount = actions
                      ? actions.press.length +
                        actions.release.length +
                        actions.long_press.length +
                        actions.double_press.length
                      : 0;
                    if (
                      actionCount === 0 &&
                      setupVerificationTimerRef.current
                    ) {
                      clearTimeout(setupVerificationTimerRef.current);
                      setupVerificationTimerRef.current = null;
                    }
            ...
            if (
              ...
              (!payload.params.button ||
                payload.params.button ===
                  setupVerificationButtonRef.current.buttonId)
            ) {
              if (setupVerificationTimerRef.current) {
                clearTimeout(setupVerificationTimerRef.current);
                setupVerificationTimerRef.current = null;
  ...
  const updateProfile = useCallback(
    ...
    [profileById, profileHistory.record],
  );
  ...
  const saveProductConfig = useCallback(
    (
      ...
      const existingMetadata = devices.find(
        (device) => device.deviceId === deviceId,
      )?.productConfig?.snapshot_metadata;
      const nextConfig =
        config.snapshot_metadata === undefined
          ? { ...config, snapshot_metadata: existingMetadata }
          : config;
      if (recordHistory) productConfigHistory.record(nextConfig.id, nextConfig);
      setRegistry((current) => ({
        ...current,
        productConfigurations: current.productConfigurations.map(
          (configuration) =>
    ...
    [
      devices,
      language,
      productConfigHistory.record,
      queue,
      replaceRegistrySnapshot,
    ],
  );

  const changeManagedActions = useCallback(
    (profile: DeviceProfile) => {
      const productDevice = devices.find(
        (device) =>
          device.deviceId === selectedManagedDeviceId &&
          device.productVersionId,
      );
  ...
  const saveUsageSettings = useCallback(
    async (settings: UsageSettingsPatch) => {
      if (PREVIEW_MODE) return;
      try {
        const snapshot = await invoke<AppSnapshot>("save_usage_settings", {
          settings,
        });
        if (mountedRef.current) applySnapshot(snapshot, true);
      } catch (operationError) {
  ...
  const completeDeviceSetup = async (
    ...
    const completed = PREVIEW_MODE
      ? {
          deviceProfiles: deviceProfiles.some(
            (profile) => profile.profile.id === assignment.device_profile_id,
          )
            ? deviceProfiles
            : [
                ...deviceProfiles,
          ...
          current.kind === "import"
            ...
            : current.kind === "restore"
              ...
              : current.kind === "delete"
                ...
                : await invoke<AppSnapshot>("forget_device", {
  ...
  const historyProfileId =
    devices.find((device) => device.deviceId === selectedManagedDeviceId)
  ...
  const historyTarget: HistoryTarget =
    selectedHistoryDevice?.productVersionId &&
    selectedHistoryDevice.productConfigurationId
      ? {
          kind: "product",
  ...
  const canUndoProfile = Boolean(
    ...
    profileHistory.canUndo(historyTarget.profileId),
  );
  const canRedoProfile = Boolean(
    historyTarget?.kind === "profile" &&
    profileHistory.canRedo(historyTarget.profileId),
  );
  const canUndoProduct = Boolean(
    historyTarget?.kind === "product" &&
    productConfigHistory.canUndo(historyTarget.configurationId),
  );
  const canRedoProduct = Boolean(
    historyTarget?.kind === "product" &&
    productConfigHistory.canRedo(historyTarget.configurationId),
  );
  ...
  return (
    <main className="product-shell">
      <header className="topbar">
        ...
        <div
          className="edit-history-actions"
          aria-label={t(language, "common.editHistory")}
        >
          <button
            ...
            onClick={() => {
              if (historyTarget?.kind === "product") {
                const config = productConfigHistory.undo(
                  historyTarget.configurationId,
                );
                if (config)
                  saveProductConfig(historyTarget.deviceId, config, false);
              } else if (historyTarget?.kind === "profile") {
                applyHistoryProfile(
                  profileHistory.undo(historyTarget.profileId),
                );
              }
            }}
          >
          ...
          <button
            ...
            onClick={() => {
              if (historyTarget?.kind === "product") {
                const config = productConfigHistory.redo(
                  historyTarget.configurationId,
                );
                if (config)
                  saveProductConfig(historyTarget.deviceId, config, false);
                ...
                applyHistoryProfile(
                  profileHistory.redo(historyTarget.profileId),
                );
              }
            }}
          >
                    ...
                    return (
                      <article className="profile-row" key={profile.profile.id}>
                        <div className="profile-row-main">
                          ...
                          {formatSnapshotDate(
                            language,
                            profile.snapshot_metadata?.created_at,
                          ) && (
                            <p>
                          ...
                          {profile.snapshot_metadata?.source_device_name && (
                            <p>
                              {t(language, "data.sourceDevice", {
                                name: profile.snapshot_metadata
                                  .source_device_name,
                              })}
                            </p>
                          )}
