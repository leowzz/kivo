...
interface DeviceManagementProps {
  ...
  onCreateProductConfiguration?(request: {
    deviceId: string;
    name: string;
    copyCurrent: boolean;
  }): Promise<void>;
  ...
  onDuplicateProfileForDevice?(request: {
    deviceId: string;
...
export function DeviceManagement({
  ...
  const [localSelectedButtonId, setLocalSelectedButtonId] = useState<
    string | null
  >(null);
  const [settingsOpen, setSettingsOpen] = useState(false);
  const [productConfigurationCreating, setProductConfigurationCreating] =
    useState(false);
  const [productConfigurationName, setProductConfigurationName] = useState("");
  const [copyCurrentProductConfiguration, setCopyCurrentProductConfiguration] =
    useState(true);
  const [productConfigurationSaving, setProductConfigurationSaving] =
    useState(false);
  ...
  const activeSelection = requestedExists
    ? requestedSelection
    : (rows[Math.max(0, Math.min(previousIndex, rows.length - 1))]?.selection ??
      null);
  useEffect(() => {
    if (
  ...
  const selectedDevice =
    activeSelection?.kind === "device"
      ? (devices.find((device) => device.deviceId === activeSelection.id) ??
        null)
      : null;
  ...
  const selectedCandidate =
    activeSelection?.kind === "candidate"
      ? (candidates.find((candidate) => candidate.key === activeSelection.id) ??
        null)
      : null;
  const selectedError =
    error &&
  ...
  const handleKeypadEscape = useCallback(() => {
    ...
    const target = deviceId
      ...
      : rows.find((row) => row.selection.kind === "device")
        ? deviceRowElements.current[
            rows.find((row) => row.selection.kind === "device")!.selection.id
          ]
        : null;
    target?.focus();
  ...
  const productEditingProfile: DeviceProfile | undefined =
    selectedDevice?.productDefinition && selectedDevice.productConfig
      ? {
          schema_version: 3,
          ...
          hardware_profiles: [
            selectedDevice.productDefinition.hardware_profile,
          ],
          actions: selectedDevice.productConfig.actions,
        }
      : undefined;
  const editingProfile =
    productEditingProfile ??
    deviceProfiles.find(
      (item) =>
        item.profile.id ===
  ...
  const starterProfiles = useMemo(
    () =>
      deviceProfiles.filter((profile) =>
        STARTER_PROFILE_IDS.has(profile.profile.id),
      ),
    [deviceProfiles],
  );
  ...
  const selectedButton =
    buttons.find((button) => button.id === effectiveSelectedButtonId) ?? null;
  const selectedActions: TriggerActions =
    editingProfile && effectiveSelectedButtonId
  ...
  const selectProductConfiguration = async (configurationId: string) => {
    ...
    setError(null);
    try {
      await onSelectProductConfiguration(
        selectedDevice.deviceId,
        configurationId,
      );
    } catch (reason) {
  ...
  return (
    <div
      className={`device-management${editingProfile ? " is-workspace" : ""}`}
    >
      <section
      ...
      >
        <header className="device-list-header">
          <div className="device-list-heading">
            ...
            {
              <span>
                {t(language, "devices.deckSummary", {
        ...
        <div className="connected-device-list">
                ...
                <div className="device-row-wrap">
                  ...
                  >
                    ...
                    <span className="device-row-meta">
                      ...
                      <span
                        className="device-identifier"
                        title={device.hardwareSerial}
                      >
                        {device.hardwareSerial}
                      </span>
                      <span
                        className={`device-connection-state ${device.connection === "online" ? "" : "is-offline"}`}
                      >
                        <i aria-hidden="true" />
                        {device.connection === "online"
                          ? t(language, "devices.connected")
                          : t(language, "device.offline")}
                      </span>
                      <span aria-hidden="true">·</span>
                      <span
                        className={`device-availability ${device.runtime === "ready" ? "is-available" : ""}`}
                      >
                        {device.runtime === "ready"
                          ? t(language, "devices.available")
                          : status(device, language)}
                      </span>
                    </span>
                  </button>
                  {onForgetDevice && device.connection === "offline" && (
                    <button
              ...
              <li key={candidate.key}>
                ...
                >
                  ...
                  <span className="device-row-meta">
                    ...
                    <span className="device-connection-state">
                      <i aria-hidden="true" />
                      {t(language, "devices.connected")}
                    </span>
                    <span aria-hidden="true">·</span>
                    <span className="device-availability is-attention">
                      {t(language, "devices.needsSetup")}
                    </span>
                  </span>
                </button>
              </li>
          ...
          {visibleDevices.length === 0 && visibleCandidates.length === 0 && (
            <p className="device-list-empty">
              {t(language, "devices.noConnected")}
            </p>
          )}
        </div>
      </section>
      ...
      >
        {!selectedDevice && !selectedCandidate && (
          <div className="panel-empty device-empty-state">
            ...
            onCreateFromTemplate ? (
              <section
                className="device-empty-templates"
                aria-labelledby="device-empty-templates-title"
              >
                <h3 id="device-empty-templates-title">
                  {t(language, "behavior.emptyTemplates")}
                </h3>
                  ...
                  {starterProfiles.map((profile) => (
                    ...
                    >
                      ...
                      <span className="setup-template-copy">
                        ...
                        <small>
                          {t(language, "devices.createFromTemplate")}
                        </small>
                      </span>
                    </button>
                  ))}
            ...
            <div className="candidate-actions">
              ...
              )}
              {
                <button
                  ...
                  type="button"
                  onClick={() =>
                    onOpenSetup(candidateSetupId(selectedCandidate))
                  }
                >
          ...
          <>
            <div className="device-detail-title">
              <div className="device-detail-heading-copy">
                {<span>{t(language, "nav.layout")}</span>}
                <div className="device-detail-name-row">
                  <h2>
                    {renaming
                      ? t(language, "devices.rename")
                      : selectedDevice.name}
                  </h2>
                  {!renaming && (
                    <button
                  ...
                  {editingProfile && (
                    <button
                      className="icon-button"
                      type="button"
                      aria-label={t(language, "devices.configurationSettings")}
                      title={t(language, "devices.configurationSettings")}
              ...
              </div>
              {
                <label className="device-configuration-selector">
                  ...
                  <select
                    ...
                    disabled={assignmentSaving || productConfigurationSaving}
                    onChange={(event) =>
                      selectTitleConfiguration(event.target.value)
                    }
                  >
                    {!selectedDevice.productVersionId && (
                    ...
                    {(selectedDevice.productVersionId
                      ? compatibleProductConfigurations.map(
                          (configuration) => ({
                            id: configuration.id,
                  ...
                  {sharedDeviceCount > 1 && selectedDevice.productVersionId && (
                    <small>
                      {t(language, "devices.sharedByDevices", {
            ...
            {editingProfile && !isProductDevice && sharedDeviceCount > 1 && (
              <div className="shared-configuration-warning" role="status">
                {t(language, "devices.sharedWarning", {
                ...
                <button
                  ...
                  type="button"
                  onClick={() => void onSaveSharedProfile?.(editingProfile)}
                >
                  {t(language, "devices.saveShared")}
                </button>
                <button
                  className="secondary-button"
                  type="button"
                  onClick={() =>
                    void onDuplicateProfileForDevice?.({
                      ...
                      name: `${editingProfile.profile.name} (${selectedDevice.name})`,
                    })
                  }
                >
                  {t(language, "devices.duplicateForDevice")}
                </button>
              </div>
            )}
            {editingProfile && (
      ...
      {editingProfile && (
        <ActionEditor
          ...
          onUseTemplate={
            selectedDevice && onDuplicateProfileForDevice
              ? (profileId) => {
                  const sourceProfile = deviceProfiles.find(
      ...
      {editingProfile && (
        <ConfigurationSettingsDialog
          ...
          allowDuplicate={!isProductDevice}
          onCancel={() => setSettingsOpen(false)}
          onSave={(settings: TriggerSettings) => {
      ...
      {productConfigurationCreating && selectedDevice && (
        <div className="modal-backdrop" role="presentation">
          ...
          >
            <header>
              <h2 id="product-configuration-create-title">
                {t(language, "productConfiguration.create")}
              </h2>
              <button
                className="icon-button"
                type="button"
            ...
            >
              <label>
                ...
                <input
                  ...
                  disabled={productConfigurationSaving}
                  onChange={(event) =>
                    setProductConfigurationName(event.target.value)
                  }
                />
              </label>
              <label className="product-configuration-copy-option">
                <input
                  ...
                  disabled={productConfigurationSaving}
                  onChange={(event) =>
                    setCopyCurrentProductConfiguration(event.target.checked)
                  }
                />
