...
import type { BoardProfileSummary, CandidateStatus, DeviceProfile, DeviceStatus } from "./types";
...
test("shows no compatible hardware state without attempting an automatic save", async () => {
  ...
  await user.click(rowByIdentifier("ESP-A-003"));
  expect(within(screen.getByRole("combobox", { name: "使用配置" })).queryByRole("option", { name: "Counter Profile" })).toBeNull();
...
test("does not save when the selected Device Profile is unchanged", async () => {
  ...
  await user.selectOptions(screen.getByRole("combobox", { name: "使用配置" }), "profile-a");
  expect(onSaveRuntimeAssignment).not.toHaveBeenCalled();
  expect(screen.queryByRole("button", { name: "保存运行分配" })).toBeNull();
});
...
test("does not turn an assigned Device into an empty draft", async () => {
  ...
  const select = screen.getByRole("combobox", { name: "使用配置" });
...
test("associates an action failure with its key in the main workspace", () => {
  const profile = structuredClone(profiles[0]);
...
test("keeps overview diagnostics for a device without an editing profile", () => {
  ...
  expect(screen.getByText("查看技术详情", { selector: "summary" })).toBeInTheDocument();
});
...
test("duplicates a legacy device configuration from its selector and retains errors for retry", async () => {
  ...
  renderManagement({ onDuplicateProfileForDevice });
