...
| 按键驱动 | P3.0–P3.3 四线扫描 + RST 独立键 | 临时固件已收到全部 17 个 ID 的按下/松开事件 |
| 触摸驱动 | TIN1、TIN5、TIN4、TIN3、TIN2 共五路 | 临时固件已连续采样；用户确认触摸测试正常，空间顺序尚未记录 |
| 灯光驱动 | PWM2 单通道，默认 P3.4 | Ver202 静态反汇编；不能视为已确认 L1 接线 |
| 官方固件 USB | `3062:4700`，名称 `丐17Touch` | Ver202 静态描述符 |
| 当前运行时 USB | `1209:C55C`，名称 `Kivo CH552 KeyTest` | 2026-09-09 已刷入临时测试固件，实机识别到键盘和 CDC |
...
- [临时测试固件与监测页](../firmware/ch552g-keytest/README.md)：按键输出 1–17、五路触摸曲线、校准、构建与恢复说明。
...
曾出现找不到 WCH ISP USB 设备的报错，用户重连后确认恢复，原因未确定。当前已刷入独立临时测试固件并完成代码校验，128 字节 EEPROM 前后相同，用户确认按键与触摸测试正常；这不代表完成 Kivo 正式固件适配、L1 驱动或触摸手势算法验收。
